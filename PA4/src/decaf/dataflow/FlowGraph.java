...
public class FlowGraph implements Iterable<BasicBlock> {
    ...
    /**
     * Edited by weiyx15 to compute usePoints for each Basic Block
     */
    public void computeUsePoints()
    {
    	for (BasicBlock bb : bbs)
    	{
    		for (BasicBlock bb2 : bbs)
    		{
    			if (bb.bbNum != bb2.bbNum)
    			{
    				for (Temp temp : bb2.innerUsePoints.keySet())
            		{
        				if (bb.usePoints.get(temp) == null)
        				{
        					bb.usePoints.put(temp, new TreeSet<Pair>(Pair.COMPARATOR));
        				}
        				if (bb2.innerUsePoints.get(temp) != null)
        					bb.usePoints.get(temp).addAll(bb2.innerUsePoints.get(temp));
            		}
    			}
    		}
    	}
    ...
    /**
     * Created by weiyx15 to support Basic Block DU chain analysis
     */
    public void analyzeLiveness() {
    	// compute liveUse, def, DUliveUse
        for (BasicBlock bb : bbs) {
        ...
        do {
            ...
            for (BasicBlock bb : bbs) {
                ...
                bb.DUliveOut.removeAll(bb.DUdef);
