...
public class GraphColorRegisterAllocator implements RegisterAllocator {
	...
	public void alloc(BasicBlock bb) {
		...
		// But before that, you have to load liveUse from stack to registers.
		Iterator<Temp> iter = bb.liveUse.iterator();
		while (iter.hasNext())
		{
			load(bb.tacList, iter.next());		// add tacs to load liveUse temps from stack
			bb.tacList.liveOut = bb.liveUse;	// create liveOut for these tacs
		}
		InferenceGraph ig = new InferenceGraph();
		ig.alloc(bb, regs, fp.reg);				// allocate registers for basic block using inference graph
	...
	private void load(Tac tac, Temp temp) {
		...
		{
			System.out.println(temp + " " + temp.offset);
			throw new IllegalArgumentException(bb.var +
					" may used before define during register allocation");
		}
