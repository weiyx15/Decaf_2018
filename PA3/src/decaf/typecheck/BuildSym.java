...
public class BuildSym extends Tree.Visitor {
	...
	// root
	/**
	 * Edit by weiyx15 to support SEALED
	 */
	@Override
	public void visitTopLevel(Tree.TopLevel program) {
		...
		for (Tree.ClassDef cd : program.classes) {
			...
			Class parent = table.lookupClass(cd.parent);
			if (parent != null && parent.isCheck())
			{
	...
	/**
	 * Edit by weiyx15
	 * If an assign stmt contains var, then a variable with unknown type is defined
	 */
	@Override
	...
	{
		assign.left.accept(this);	// assign.left could be "var Tree.Ident"
		assign.expr.accept(this);	// but assign.right could not
	}
	
	/**
	 * Edit by weiy15 to open new local scope
	 */
	@Override
	...
	/**
	 * Edit by weiyx15 to support foreach variable definition
	 */
	@Override
	public void visitBoundVariable(Tree.BoundVariable bv)
	{
		if (bv.type == null)	// var Tree.Ident
		{
			Variable v = new Variable(bv.name, BaseType.UNKNOWN, 
					bv.getLocation());
			table.declare(v);
			bv.symbol = v;
		}
		else					// Type Tree.Ident
		{
			bv.type.accept(this);
			Variable v = new Variable(bv.name, bv.type.type, 
					bv.getLocation());
			table.declare(v);
			bv.symbol = v;
		}
