...
public class Translater {
	...
	/**
	 * Edit by weiyx15
	 * support division by zero check
	 */
	public Temp genDiv(Temp src1, Temp src2) {
		Label ok = Label.createLabel();
		Temp zero = genLoadImm4(0);
		Temp cond = genEqu(src2, zero);	// cond == 1, division by zero err
		genBeqz(cond, ok);
		Temp msg = genLoadStrConst(RuntimeError.DVISION_BY_ZERO);
		genParm(msg);
	...
	/**
	 * Edit by weiyx15 to support "E %% n" array size check
	 * @param size
	 */
	public void genCheckCreatedArraySize(Temp size) {
		Label exit = Label.createLabel();
		Temp cond = genLes(size, genLoadImm4(0));
		genBeqz(cond, exit);
		Temp msg = genLoadStrConst(RuntimeError.NEGATIVE_CREATED_ARR_SIZE);
		genParm(msg);
		genIntrinsicCall(Intrinsic.PRINT_STRING);
		genIntrinsicCall(Intrinsic.HALT);
		genMark(exit);
	...
	/**
	 * Edit by weiyx15 for TransPass2.visitScopy
	 * @param dst
	 * @param src
	 * @param size
	 */
	public void genScopy(Temp dst, Temp src, int size)
	{
		Temp tsize = genLoadImm4(size);
		Temp unit = genLoadImm4(OffsetCounter.WORD_SIZE);						// unit size
		Temp i = genLoadImm4(OffsetCounter.WORD_SIZE);							// attribute iterator
		Label check = Label.createLabel();
		Label exit = Label.createLabel();
		genMark(check);
		Temp cond = genLes(i, tsize);
		genBeqz(cond, exit);
		Temp base = genAdd(src, i);
		Temp attr = genLoad(base, 0);
		base = genAdd(dst, i);
		genStore(attr, base, 0);
		Temp addi = genAdd(i, unit);
		genAssign(i, addi);
		genBranch(check);
		genMark(exit);
	}
	 ...
	 * @param elem
	 * @param length
	 * @return array of elements with length
	 * TODO
	 */
	public Temp genCreatedArray(Temp elem, Temp length, boolean isClass, Label cLabel, int sizeClass) {
		genCheckCreatedArraySize(length);
		Temp unit = genLoadImm4(OffsetCounter.WORD_SIZE);
		Temp size = genAdd(unit, genMul(unit, length));		// array size
		genParm(size);
		Temp obj = genIntrinsicCall(Intrinsic.ALLOCATE);
		genStore(length, obj, 0);
		Label loop = Label.createLabel();
		...
		if (isClass)
		{
			Temp obj1 = genDirectCall(cLabel, BaseType.INT);
			genScopy(obj1, elem, sizeClass);
			genStore(obj1, obj, 0);
		}
		else
		{
